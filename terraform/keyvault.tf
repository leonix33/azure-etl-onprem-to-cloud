...
resource "azurerm_key_vault_secret" "sql_connection_string" {
  ...
  key_vault_id    = azurerm_key_vault.etl_kv.id
  expiration_date = "2026-02-12T23:59:59Z" # 5 days ago (EXPIRED status)

  depends_on = [azurerm_key_vault_access_policy.terraform_policy]
}

# Store Azure AI Search admin key
# Status: 🟡 WARNING - Expires in 15 days
resource "azurerm_key_vault_secret" "search_admin_key" {
  ...
  key_vault_id    = azurerm_key_vault.etl_kv.id
  expiration_date = "2026-03-04T23:59:59Z" # 15 days from now (WARNING status)

  depends_on = [azurerm_key_vault_access_policy.terraform_policy]
}

# Store Azure OpenAI key
# Status: 🔵 NOTICE - Expires in 60 days
resource "azurerm_key_vault_secret" "openai_api_key" {
  name            = "openai-api-key"
  value           = azurerm_cognitive_account.etl_openai.primary_access_key
  key_vault_id    = azurerm_key_vault.etl_kv.id
  expiration_date = "2026-04-18T23:59:59Z" # 60 days from now (NOTICE status)

  depends_on = [azurerm_key_vault_access_policy.terraform_policy]
}

# Demo secret for testing dashboard
# Status: ⚪ NO EXPIRY - Never expires (should be flagged)
resource "azurerm_key_vault_secret" "demo_no_expiry" {
  name         = "demo-secret-no-expiry"
  value        = "DemoValueNoExpiration"
  key_vault_id = azurerm_key_vault.etl_kv.id
